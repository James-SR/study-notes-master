...
In this exercise, you'll just create your objects and arrange them using grid.arrange()
...
Next we want to extract and better locate the legend.  
...
Since ggplot2 2.0 you can now write your own extensions, meaning we can combine statistics and plotting.  Here we will create a bag plot in ggplot2, which is like a 2 dimensional box plot when dealing with 2 variables.  To calculate the elements required for the bag plot we will use the aplpack package.  Using the extensions means that we can then use this new function for other data.  First we will explore what a bag plot is.  

```{r}
# Load the library and test data
library(aplpack)
...
Let's see how this simple solution works and in the next exercises you'll expand on this topic to make a real stats layer. We will build the plot up incrementally, so it might not be the best looking plot to begin with.
...
Now we need to combine them under a new ggplot2 function that you'll call stat_bag().
...
### Weather Case Study - Pt1

In this exercise we will be looking at creating something similar to the Edward Tufte weather plot created for the New York Times.  WE want to look at the current year's daily high and lows compared to historical averages for any particular day.  In this case, we can use a 95% CI for historical records.  
...
```{r}
# Import weather data
weather <- read.fwf("D:/CloudStation/Documents/2017/RData/NYNEWYOR.txt",
                    header = FALSE,
...
str(past_summ)
```
Now we can create our historical plot
...
Next we want to create our data frame for the latest lear and add this as a new layer on to out plot.
...
The first method, which you'll use here and in the next exercise, works fine, but you'll explore how to make it more efficient later.
...
Next we combine the previous two exercises to identify the record highs and lows in one step rather than two (one for highs and lows), assign them to a new data frame called extremes, and use this to map a color aesthetic. This will make both your data munging and plotting code more efficient.
...
Now we can setup the custom legend using the viewport function from the grid package.  
...
### Weather Case Study - Pt2

Here we will look at making our process and code more efficient.  Now that we have created a unique visualization to answer a specific data analysis question, it would be nice if we can make our lives even easier.  For instance, we can generalise our functions to other cities.

Instead of trying to combine all steps into a single function, we will make several stat functions which will act as layers in ggplot2 plots. This will help you to make flexible plots in the future.  We will first create a clean_weather() function which we can then point at the relevant file.
...
my_data <- clean_weather("D:/CloudStation/Documents/2017/RData/NYNEWYOR.txt")
```

Next we develop the first layer to show the historical data called stat_historical(). Like before, we create a stats layer starts by creating a stats object with ggproto(), and then we create a function where you'll define a number of layers.

To do this properly, you are going to define a new aesthetic inside ggproto(). This will be called "year", and you can map the variable year from your dataset onto this aesthetic. 
...
Next we want to add the current year. We need to do the same trick of defining a new "year" aesthetic and then filter the dataset accordingly. This time you only need one layer, there's no notion of a line range geom in this case.
...
You're at the end of your case study: you have built up three stat_ functions that you can use in any combination you like and with any dataset that fits the type of stats you are doing. Let's try it out on some other cities. There are four files, related to the weather data for New York, Paris, Reykavik and London. Their filenames are stored in the my_files vector.

```{r}
# File paths of all datasets
setwd("D:/CloudStation/Documents/2017/RData")
my_files <- c("NYNEWYOR.txt",
              "FRPARIS.txt", 
              "ILREYKJV.txt", 
              "UKLONDON.txt")
