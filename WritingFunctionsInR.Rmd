...
map_dbl(coefs, 1)
```
purrr also includes a pipe operator: %>%. The pipe operator is another shortcut that saves typing, but also increases readability.  The example below pulls out the R2R2 from each model. Rewrite the last two lines to use a pipe instead.
